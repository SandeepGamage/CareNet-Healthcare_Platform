...
export default function Prescriptions() {
  ...
  const [testMedication, setTestMedication] = useState("");
  const [testPatientId, setTestPatientId] = useState("");
  const [testReportId, setTestReportId] = useState("");
  const [testFormResult, setTestFormResult] = useState("");
  ...
  const fetchDoctorPatients = async () => {
    ...
    try {
      ...
      setPatientsData([]);
      setErrorMessage("Could not load appointment patients from backend.");
    }
  };
  ...
  const handleTestFormSubmit = async (e) => {
    ...
    try {
      const token = getAuthToken();
      if (!token) {
        setTestFormResult("Login token not found. Please login again.");
      ...
      setTestFormResult("Success: " + JSON.stringify(response.data));
    } catch (error) {
      setTestFormResult("Error: " + (error.response?.data?.message || error.message));
    } finally {
      setTestFormLoading(false);
    }
  };
      ...
      <div className="p-8">
        <>
          ...
          <div className="mb-8 p-6 bg-yellow-50 border border-yellow-200 rounded-xl">
            <h2 className="font-bold text-lg mb-2 text-yellow-800">Test Prescription Form (Direct API)</h2>
            <form className="flex flex-col gap-3" onSubmit={handleTestFormSubmit}>
              ...
              <button
                type="submit"
                className="bg-yellow-600 hover:bg-yellow-700 text-white font-semibold px-4 py-2 rounded-lg w-fit"
                disabled={testFormLoading}
              >
                {testFormLoading ? "Submitting..." : "Submit Test Prescription"}
              </button>
            </form>
            {testFormResult && (
              <div className="mt-3 text-sm whitespace-pre-wrap break-all">
                {testFormResult}
              </div>
            )}
          </div>
